...
## Output Location
All CSVs write to `event-scraping-bot/output/` (gitignored).
...
## API Wrappers
All wrappers live in `applications/`:
- `applications/firmable.py` — `FirmableClient` (lookup_company, search_by_linkedin, find_contacts)
- `applications/smartlead.py` — `SmartLeadClient` (add_leads_to_campaign)
- `applications/hubspot.py` — `HubSpotClient` (create_or_update_contact)
- `applications/ai.py` — `ask_claude`, `ask_claude_with_vision`
...
## Key References
- `knowledge/icp-criteria.md` — scoring rubric (sales team size, industry, competitive tech)
- `knowledge/firmable-context.md` — ICP definition, value prop
