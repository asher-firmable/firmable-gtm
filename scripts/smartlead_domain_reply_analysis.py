...
For every sending domain across all mailboxes, shows:
  ...
  - Reply rate (14d): replies (14d) / sends from active campaigns (all-time)
  - Reply rate (all-time): all-time replies / all-time sends across ALL campaigns
...
date-filtered sent API). The 14d rate is an approximation; the all-time
rate uses campaign analytics reply_count across all campaigns (active +
completed) and is the best indicator of whether a domain is burned.
...
def fetch_alltime_stats_per_account(client: SmartLeadClient, campaigns: list) -> tuple:
    ...
    for i, camp in enumerate(campaigns, 1):
        ...
        print(f"  [{i}/{len(campaigns)}] {camp_name[:60]}", end="\r", flush=True)
...
def write_html_report(
    ...
    def mailbox_rows_html(domain):
        ...
        for acc_id in acc_ids:
            ...
            if sent > 0:
                rate_14d = replies / sent * 100
                rate_14d_str = f"{rate_14d:.2f}%"
                rate_14d_cls = _reply_rate_class(rate_14d)
            else:
                rate_14d = -1
                rate_14d_str = "—"
                rate_14d_cls = "rate-none"
            ...
            html.append(
                ...
                f'<td class="mono num rate-cell {rate_14d_cls}" data-val="{rate_14d:.4f}">{rate_14d_str}</td>'
    ...
    domain_rows_html = []
    for r in rows:
        rate_14d = r["reply_rate"]
        rate_14d_str = f"{rate_14d:.2f}%" if rate_14d is not None else "—"
        rate_14d_cls = _reply_rate_class(rate_14d)
        rate_14d_val = rate_14d if rate_14d is not None else -1
        ...
        domain_rows_html.append(
            ...
            f'{camp_count_cell}'
            f'<td class="mono num rate-cell {rate_14d_cls}" data-val="{rate_14d_val}">{rate_14d_str}</td>'
...
<style>
  ...
  *,*::before,*::after {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .mbx-rec-list {{ overflow-x:auto; margin-top:12px; }}
  .mbx-rec-row {{ display:flex; align-items:center; gap:8px; flex-wrap:nowrap;
    padding:7px 0; border-bottom:1px solid var(--border); min-width:max-content; }}
