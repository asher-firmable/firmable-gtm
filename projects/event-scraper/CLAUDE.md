...
### 2. Run the scraper
```bash
PYTHONPATH=. python3 projects/event-scraper/scripts/0_scrape_exhibitors.py \
  --url "EVENT_SPONSORS_PAGE_URL" \
  --output "projects/event-scraper/output/EVENT_NAME_raw.csv"
```

### 3. Run subsequent steps
```bash
PYTHONPATH=. python3 projects/event-scraper/scripts/1_find_contacts.py \
  --input "projects/event-scraper/output/EVENT_NAME_raw.csv" \
  --output "projects/event-scraper/output/EVENT_NAME_contacts.csv"
```

### 4. Or run all at once
```bash
PYTHONPATH=. python3 projects/event-scraper/scripts/run_all.py --project [event_name]
```

---

## Output Location
All CSVs write to `projects/event-scraper/output/` (gitignored).
